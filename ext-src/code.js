...
function GUI() {
    ...
    cheatMenuBtn.click();
    cheatMenu = document.getElementById(n[0]);
    newCheatMenu = `
    <div>
    <div class="close">
       <button class="btn-red" id="c">Close</button>
    </div>
    <h2 style="margin: 0px;">Advanced Dredark Tools</h2>
    <table>
       <tr>
          <td>Anti-AFK</td>
          <td>
             <button class="btn-small btn-red" id="Anti-AFK">Toggle</button>
             <button class="btn-small btn-yellow" id="o1">Settings</button>
          </td>
       </tr>
       <tbody id="o111" style="display:none">
          <tr>
             <td>
             <td>
             <td>
                <p>
                   <text id="o11">Delay: 5 Sec.</text>
                   <input id="o1111"class="center-vert" type="range" min="1" max="120" style="width: 200px;" value="5">
                </p>
                <p>
                   <input id="o11111" type="checkbox">Advanced AFK
                </p>
             </td>
             </td>
             </td>
       ...
       <tr>
          <td>HealBot Mode</td>
          <td>
             <button id="o2" class="btn-small btn-red">Toggle</button>
             <button class="btn-small btn-yellow" id="o21">Settings</button>
          </td>
       <tbody id="o222" style="display:none">
          <tr>
             <td>
             <td>
             <td>
                <p>
                   <input id="o2222" type="checkbox">SmartHeal(Syncs Bots)
                </p>
             </td>
             </td>
             </td>
       ...
       <tr>
          ...
          <td>
             ...
             <button class="btn-small btn-yellow" id="o31">Settings</button>
          </td>
          <tbody id="o33" style="display:none">
          <tr>
             <td>
             <td>
             <td>
...
setTimeout(function() {
    ...
    function downKey(e) {
        ...
        if (key === 't' && !pressed) {
            ...
            */
            ...
            //healy = tempy;
            //alert("jumping");
            //document.getElementsByTagName('input')[4].value=names[0];
            //alert(document.getElementsByTagName('input')[4].value);
            //chrome.runtime.sendMessage({ eventPlease: "trusted", x: cx, y: cy, mouse: "J" }, function (response) {console.log(response.yourEvent);});
            //chrome.runtime.sendMessage({ eventPlease: "trusted", x: cx, y: cy, mouse: "D" }, function (response) {console.log(response.yourEvent);});

            //setInterval(function(){chrome.runtime.sendMessage({ eventPlease: "trusted", x: cx, y: cy, mouse: "D" }, function (response) {console.log(response.yourEvent);}}, 100);
        } else if (key === 'r') {
            //chrome.runtime.sendMessage({ eventPlease: "trusted", x: cx, y: cy, mouse: "D" }, function (response) {console.log(response.yourEvent);});
    ...
    function upKey(e) {
        ...
        if (key === 't' && pressed) {
            e.preventDefault();
            //pressed = false;
            //chrome.runtime.sendMessage({ eventPlease: "trusted", x: cx, y: cy, mouse: "U" }, function (response) {console.log(response.yourEvent);});
        } else if (key === 'r') {
            let clickEvent = document.createEvent('MouseEvents');
